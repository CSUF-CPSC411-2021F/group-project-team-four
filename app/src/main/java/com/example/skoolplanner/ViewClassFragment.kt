...
class ViewClassFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val viewModelFactory = ClassScheduleViewModelFactory(dataSource, application)

        // Generate an IntersectionViewModel using the factory.
        val classScheduleViewModel =
            ViewModelProvider(
                this, viewModelFactory).get(ClassScheduleViewModel::class.java)

        // Connect the IntersectionViewModel with the variable in the layout
        binding.classScheduleViewModel = classScheduleViewModel
        ...
        classScheduleViewModel.classScheduleList.observe(viewLifecycleOwner, Observer {
            it?.let {
